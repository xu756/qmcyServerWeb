...
var (
	userRoleFieldNames          = builder.RawFieldNames(&UserRole{}, true)
	userRoleRows                = strings.Join(userRoleFieldNames, ",")
	userRoleRowsExpectAutoSet   = strings.Join(stringx.Remove(userRoleFieldNames), ",")
	userRoleRowsWithPlaceHolder = builder.PostgreSqlJoin(stringx.Remove(userRoleFieldNames, "id"))

	cachePublicUserRoleIdPrefix = "cache:public:userRole:id:"
)
...
func (m *defaultUserRoleModel) Delete(ctx context.Context, id int64) error {
	publicUserRoleIdKey := fmt.Sprintf("%s%v", cachePublicUserRoleIdPrefix, id)
...
func (m *defaultUserRoleModel) FindOne(ctx context.Context, id int64) (*UserRole, error) {
	publicUserRoleIdKey := fmt.Sprintf("%s%v", cachePublicUserRoleIdPrefix, id)
	var resp UserRole
	err := m.QueryRowCtx(ctx, &resp, publicUserRoleIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
		query := fmt.Sprintf("select %s from %s where id = $1 limit 1", userRoleRows, m.table)
...
func (m *defaultUserRoleModel) Insert(ctx context.Context, data *UserRole) (sql.Result, error) {
	publicUserRoleIdKey := fmt.Sprintf("%s%v", cachePublicUserRoleIdPrefix, data.Id)
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("insert into %s (%s) values ($1, $2, $3, $4, $5, $6, $7, $8)", m.table, userRoleRowsExpectAutoSet)
		return conn.ExecCtx(ctx, query, data.Id, data.UserId, data.RoleId, data.Created, data.Creator, data.Edited, data.Editor, data.Deleted)
	}, publicUserRoleIdKey)
	return ret, err
}
...
func (m *defaultUserRoleModel) Update(ctx context.Context, data *UserRole) error {
	publicUserRoleIdKey := fmt.Sprintf("%s%v", cachePublicUserRoleIdPrefix, data.Id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("update %s set %s where id = $1", m.table, userRoleRowsWithPlaceHolder)
		return conn.ExecCtx(ctx, query, data.Id, data.UserId, data.RoleId, data.Created, data.Creator, data.Edited, data.Editor, data.Deleted)
	}, publicUserRoleIdKey)
	return err
}
...
func (m *defaultUserRoleModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cachePublicUserRoleIdPrefix, primary)
}
